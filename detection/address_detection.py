...
def detect_address_errors(street, street_number, zipcode, city):
    ...
    if should_detect('4101', error_config) and rule_condition:
        ...
        if should_detect('4106', error_config) and rule_condition:
        ...
        if should_detect('4103', error_config) and rule_condition:
        ...
        if should_detect('4107', error_config) and rule_condition:
            street_errors.add('4107') 
        
        # 4109 Only numbers
        rule_condition = (not any(char.isalpha() for char in street))
        if should_detect('4109', error_config) and rule_condition:
        ...
        if street:
            ...
            for comp in components:
                ...
                if should_detect('4110', error_config) and rule_condition:
        ...
        if should_detect('4105', error_config) and rule_condition:
        ...
        rule_condition = (re.search(r'\d+$', street))
        if should_detect('4112', error_config) and skip_if_condition and rule_condition:
        ...
        if should_detect('4108', error_config) and skip_if_condition and rule_condition:
        ...
        if should_detect('4113', error_config) and skip_if_condition and rule_condition:
    ...
    if should_detect('4201', error_config) and rule_condition:
        ...
        if should_detect('4208', error_config) and rule_condition:
        ...
        if should_detect('4202', error_config) and rule_condition:
        ...
        if should_detect('4213', error_config) and rule_condition:
        ...
        if should_detect('4203', error_config) and skip_if_condition and rule_condition:
        ...
        if should_detect('4205', error_config) and skip_if_condition and rule_condition:
            street_number_errors.add('4205') 
        
        # 4210 More than one number present
        rule_condition = (len(re.findall(r'\d+', street_number)) > 1)
        if should_detect('4210', error_config) and rule_condition:
        ...
        if should_detect('4204', error_config) and skip_if_condition and rule_condition:
        ...
        if should_detect('4208', error_config) and skip_if_condition and rule_condition:
        ...
        if should_detect('4211', error_config) and skip_if_condition and rule_condition:
        ...
        if should_detect('4207', error_config) and skip_if_condition and rule_condition:
            street_number_errors.add('4207')
        
        # 4206 Leading 0
        skip_if_condition = (not '4204' in street_number_errors)
        rule_condition = (re.search(r'\b0\s*\d+', street_number))
        if should_detect('4206', error_config) and skip_if_condition and rule_condition:
    ...
    if should_detect('4301', error_config) and rule_condition:
        zipcode_errors.add('4301')
    else:
        # 4305 Check for more than 4 digits
        rule_condition = (len(re.findall(r'\d', zipcode)) > 4)
        if should_detect('4305', error_config) and rule_condition:
            zipcode_errors.add('4305')
            
        # 4304 Check for less than 4 digits
        rule_condition = len(re.findall(r'\d', zipcode)) < 4
        if should_detect('4304', error_config) and rule_condition:
        ...
        if should_detect('4302', error_config) and rule_condition:
            zipcode_errors.add('4302') 
            
        # 4303 Check for invalid characters
        rule_condition = (not '4302' in zipcode_errors and not re.search(r'^\d+$',zipcode))
        if should_detect('4303', error_config) and rule_condition:
        ...
        if should_detect('4307', error_config) and skip_if_condition and rule_condition:
    ...
    if should_detect('4401', error_config) and rule_condition:
        ...
        if should_detect('4406', error_config) and rule_condition:
        ...
        if city:
            ...
            for comp in components:
                rule_condition = (prev_comp and prev_comp == comp)
                if should_detect('4407', error_config) and rule_condition:
