...
def detect_phone_errors(email):
    ...
    if pd.isna(phone) or phone is None or phone.strip() == "" or phone.strip() == "/" :
        ...
        if (
            ...
            errors.append('3104')
            distinct_detected_errors.add('3104')
        
        # 3103 Check for invalid characters
        if not re.search(r'^[0-9]+$', phone):
            errors.append('3103')
            distinct_detected_errors.add('3103')
        ...
        if (
            ...
            errors.append('3107')
            distinct_detected_errors.add('3107')
        # 3108 Check for different country format
        if phone.startswith('00'):
            errors.append('3108')
            distinct_detected_errors.add('3108')  
