...
def run_name_pipeline(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Write the results of the detect_name_errors function to the DataFrame
    ...
    # Step 2: Correct if errors detected
    '''
    define a row correction function which will be applied to each row if there are 
    errors and if there are no errors then it will return empty lists and empty errors
    for both name and surname
    '''
    def correct(row):
