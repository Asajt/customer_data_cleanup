    ...
    for index in df.index:
        ...
        if address_error_chance:
            ...
            if street_error_chance:
                ...
                if re.search(r'\d+\.', str(current_value)):
                    if np.random.rand() < 0.03:
            ...
            if house_number_error_chance:
                ...
                # If missing data was introduced, skip further errors for this row
                if "4201" not in df.at[index, "INTRODUCED_ERRORS"]:
                    ...
                    if np.random.rand() < 0.02 and current_value.isdigit() and 2 <= len(current_value) <= 3:
            ...
            if postal_code_error_chance:
                ...
                if "4301" not in df.at[index, "INTRODUCED_ERRORS"]:
                    ...
                    if np.random.rand() < 0.02:
                        new_value = np.random.choice(["ABC", "0000", "99999", "9999"])
                        if new_value != current_value:
